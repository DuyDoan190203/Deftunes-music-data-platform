...
resource "aws_glue_job" "rds_ingestion_etl_job" {
  ...
  command {
    name            = "glueetl"
      # Set the value of scripts_bucket and "deftunes-extract-songs-job.py" for the script object key
  script_location = "s3://${var.scripts_bucket}/deftunes-extract-songs-job.py"
    python_version  = 3
  }
...
resource "aws_glue_job" "api_users_ingestion_etl_job" {
  ...
  command {
    name            = "glueetl"
    # Set the value of scripts_bucket and `"deftunes-api-extract-job.py"` for the script object key
    script_location = "s3://${var.scripts_bucket}/deftunes-api-extract-job.py"
    python_version  = 3
  }
  ...
  default_arguments = {
    ...
    "--api_url"             = "http://ec2-52-54-43-81.compute-1.amazonaws.com/users"
...
resource "aws_glue_job" "api_sessions_ingestion_etl_job" {
  ...
  default_arguments = {
    ...
    "--api_end_date"        = "2020-01-31"
    # Replace the placeholder <API-ENDPOINT> with the value from the CloudFormation outputs
    "--api_url"             = "http://ec2-52-54-43-81.compute-1.amazonaws.com/sessions"
    # Notice the target path. This line of the code code is complete - no changes are required
