...
def deftunes_songs_pipeline():
    ...
    rds_extract_glue_job = GlueJobOperator(
        ...
        job_name="deftunes-rds-extract-job",
        # Location in the S3 bucket that you used to deploy your scripts
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/deftunes-extract-songs-job.py",
        ...
        script_args={
            ...
            "--rds_connection": "deftunes-pipeline-connection-rds",
            "--ingest_date": "{{ next_ds }}",
        },
    )
    ...
    songs_transform_glue_job = GlueJobOperator(
        ...
        job_name="deftunes-songs-transform-job",
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/deftunes-transform-songs-job.py",
        job_desc="Glue Job to transform songs data from RDS",
        iam_role_name="deftunes-pipeline-glue-role",
        s3_bucket=f"{SCRIPTS_BUCKET_NAME}",
        region_name="us-east-1",
        create_job_kwargs={
