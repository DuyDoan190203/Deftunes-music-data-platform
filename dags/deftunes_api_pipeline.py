...
DATA_BUCKET_NAME = "deftunes-pipeline-data-lake"
SCRIPTS_BUCKET_NAME = "deftunes-pipeline-scripts"
API_URL = "ec2-184-73-153-171.compute-1.amazonaws.com"
...
def deftunes_pipeline():
    ...
    api_users_extract_glue_job = GlueJobOperator(
        ...
        job_name="deftunes-api-users-extract-job",
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/deftunes-api-extract-job.py",
        job_desc="Glue Job to extract data from User's API endpoint",
        # The name of the role from the Terraform output `glue_role_arn`.
        iam_role_name="deftunes-pipeline-glue-role",
    ...
    api_sessions_extract_glue_job = GlueJobOperator(
        ...
        job_name="deftunes-api-sessions-extract-job",
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/deftunes-api-extract-job.py",
        job_desc="Glue Job to extract data from Sessions API endpoint",
        iam_role_name="deftunes-pipeline-glue-role",
        s3_bucket=f"{SCRIPTS_BUCKET_NAME}",
        region_name="us-east-1",
        create_job_kwargs={
    ...
    json_transform_glue_job = GlueJobOperator(
        ...
        job_name="deftunes-json-transform-job",
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/deftunes-transform-json-job.py",
        job_desc="Glue Job to transform JSON data from API endpoints",
        iam_role_name="deftunes-pipeline-glue-role",
        s3_bucket=f"{SCRIPTS_BUCKET_NAME}",
        region_name="us-east-1",
        create_job_kwargs={
        ...
        script_args={
            "--catalog_database": "deftunes_pipeline_silver_db",
            "--ingest_date": "{{ next_ds }}",
    ...
    dq_check_users_job = GlueDataQualityRuleSetEvaluationRunOperator(
        ...
        role="arn:aws:iam::157502853664:role/deftunes-pipeline-glue-role",
        ...
        datasource={
            "GlueTable": {
                "TableName": "users",
                "DatabaseName": "deftunes_pipeline_silver_db",
            }
        },
    )
